...
![Profiles](https://img.shields.io/badge/profiles-8-blue?style=flat-square)
...
이 한 줄이 **8개 프로필을 `~/.gjc/agent/models.yml` 에 안전 병합**하고 기본 프로필을 `daily` 로 설정한다. 기존 설정은 자동 백업되며, 재실행해도 깔끔히 갱신된다.
...
설치기가 하는 일: `~/.gjc/agent/models.yml` 에 8개 프로필 안전 병합(재실행 시 자동 갱신) · 기존 파일 자동 백업 · 기본 프로필 `daily` 설정. `curl` + `python3` 만 있으면 된다.
